...
$CLI -i pdb:1ark -o $DIR/1ark_models_5_test.pdb --non_interactive models --select_model 5 &> $DIR/1ark_models_test_5.log
$CLI -i pdb:1ark -o $DIR/1ark_models_1_test.pdb --non_interactive models --select_model 1 &> $DIR/1ark_models_test_1.log
#chains
#echo "Running chains on 2ki5"
$CLI -i pdb:2ki5 -o $DIR/2ki5_chains_test_all.pdb --non_interactive chains --select_chain All &> $DIR/2ki5_chains_test_all.log
$CLI -i pdb:2ki5 -o $DIR/2ki5_chains_test_B.pdb --non_interactive chains --select_chain B &> $DIR/2ki5_chains_test_B.log
#inscodes
...
$CLI -i pdb:2ki5 -o $DIR/2ki5_altloc_test_occ.pdb --non_interactive altloc --select_altloc occupancy &> $DIR/2ki5_altloc_test_occ.log
$CLI -i pdb:2ki5 -o $DIR/2ki5_altloc_test_B.pdb --non_interactive altloc --select_altloc B &> $DIR/2ki5_altloc_test_B.log
#ligands
...
$CLI -i pdb:2ki5 --non_interactive remwat --remove No &> $DIR/2ki5_remwat_test_None.log
$CLI -i pdb:2ki5 -o $DIR/2ki5_remwat_test_Yes.pdb --non_interactive remwat --remove Yes &> $DIR/2ki5_remwat_test_Yes.log
#remh
#echo "Running remh on 1ark"
$CLI -i pdb:1ark --non_interactive remh --remove No &> $DIR/1ark_remh_test_None.log
$CLI -i pdb:1ark -o $DIR/1ark_remh_test_Yes.pdb --non_interactive remh --remove Yes &> $DIR/1ark_remh_test_Yes.log
#ss bonds
...
$CLI -i $REF/chiral_pdb_test/1ubq_chi.pdb -o $DIR/1ubq_chi_chiral_test.pdb --non_interactive chiral --fix All &> $DIR/1ubq_chi_chiral_test_All.log
$CLI -i $REF/chiral_pdb_test/1ubq_chi.pdb --non_interactive chiral --fix None &> $DIR/1ubq_chi_none_test_None.log
#mutateside
...
$CLI -i $REF/chiral_pdb_test/1ark_m1_chica_nh.pdb -o $DIR/1ark_chiral_bck_test.pdb --non_interactive chiral_bck &> $DIR/1ark_chiral_bck_test.log
#backbone
...
do
  if [ -s $f ]; then
    cp $f $REF/file_a
    cp $REF/$(basename $f) $REF/file_b
    sed -i '/Structure exists/d' file_a
    sed -i '/Structure exists/d' file_b
    sed -i '/Structure saved on/d' file_a
    sed -i '/Structure saved on/d' file_b
    sed -i '/Downloading PDB structure/d' file_a
    sed -i '/Downloading PDB structure/d' file_b
    sed -i '/Structure .* loaded/d' file_a
    sed -i '/Structure .* loaded/d' file_b
    sed -i '/Summary data saved on/d' file_a
    sed -i '/Summary data saved on/d' file_b
    diff file_a file_b &> $DIR/$(basename $f).diff
    if [ -s $DIR/$(basename $f).diff ]
    then
      echo $DIR/$(basename $f)
      cat $DIR/$(basename $f).diff
    fi
    rm $REF/file_a $REF/file_b
  fi
done
