...
def _check(strcheck):
    print(cts.MSGS['MODELS_FOUND'].format(strcheck.strucm.nmodels))
    strcheck.summary['models'] = {'nmodels': strcheck.strucm.nmodels}
    if strcheck.strucm.nmodels == 1:
        if not strcheck.args['quiet']:
            print(cts.MSGS['SINGLE_MODEL'])
        return {}

    strcheck.summary['models']['type'] = strcheck.strucm.models_type
    supimp = ''
    if not strcheck.strucm.has_superimp_models():
        supimp = 'do not'
    print(
        cts.MSGS['MODELS_GUESS'].format(
            supimp,
            strcheck.strucm.models_type['rmsd'],
            mu.MODEL_TYPE_LABELS[strcheck.strucm.models_type['type']]
        )
    )
    return True
...
def _fix(strcheck, opts, fix_data=None):
    ...
    input_line.add_option_numeric(
        'modelno', [], opt_type='int', min_val=1, max_val=strcheck.strucm.nmodels, multiple=True
    )
